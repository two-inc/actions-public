...
jobs:
  code-review:
    ...
    steps:
      # Mints a short-lived Linear token via WIF, exposed only as the step's
      # masked `token` output. No GCP credential is ever created on the
      # runner (access-token-only auth, no ADC file, no exported GCP env).
      # See "Linear integration" below.
      - name: Mint Linear token
        id: linear-token
      ...
      # The token is passed explicitly to this step only — it never enters
      # the job-wide environment, so no other step can read it.
      - name: Claude review
        uses: two-inc/actions-public/claude-reviewer@main
        env:
          LINEAR_API_KEY: ${{ steps.linear-token.outputs.token }}
        with:
...
As in the template above, a `two-inc/actions/linear-token@main` step (with `id: linear-token`) placed before the `claude-reviewer` step (in a job with `id-token: write`) authenticates to GCP via Workload Identity Federation as `gha-linear-token-minter@tillit-api.iam.gserviceaccount.com` and mints a short-lived Linear OAuth token, exposed only as the step's masked `token` output. The reviewer step receives it explicitly via `env: LINEAR_API_KEY: ${{ steps.linear-token.outputs.token }}` — the token is scoped to that one step and never enters the job-wide environment, so no other step can read it. Nor does any GCP credential ever exist on the runner: WIF auth is access-token-only (no ADC credentials file is written, no GCP environment variables are exported), and the OAuth client secret is fetched over the Secret Manager REST API without touching disk. The reviewer step therefore only ever sees the masked short-lived token — never the client secret, never any GCP credential. No persisted Linear secret is needed in GitHub (`LINEAR_CLIENT_ID` is a public org variable).
